  ...
  return (
    ...
    <form  className='flex flex-col gap-5 border-2 border-[#1c1b1b] p-5 bg-[#1c1b1b] shadow-[#000000] shadow-2xl rounded-sm' onSubmit={handleSubmit(onSubmit)}>
      <div className='flex gap-4 justify-between'>
        ...
        <input type="text" placeholder='Enter name' {...register('name', { required: 'Name is required'})} className=" mb-2 py-2 px-2 bg-opacity-20 text-[#c4c3c3] border-0 border-b-2 border-[#888787] focus:border-[#888787] focus:border-b-0 bg-black" />
      </div>
      {errors.name && <p className='text-red-400'>{errors.name.message}</p>}
      <div className='flex gap-4 justify-between'>
        <label className="text-[#ffffff] ">Quantity:</label>
        <input type="number" placeholder='Enter quantity' {...register('available', { required: 'Enter quantity'})} className=" mb-3 py-2 px-2 bg-opacity-20 text-[#c4c3c3] border-0 border-b-2 border-[#888787] focus:border-[#888787] focus:border-b-0 bg-black" />
      </div>
